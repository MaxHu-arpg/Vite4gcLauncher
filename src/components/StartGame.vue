...
watch(([realIPaddress, realPort, execpath, ProxyInstalled]), ([newW, newX, newY, newZ],[oldW, oldX, oldY, oldZ]) => {
  ...
  if (newW){window.elec.setConfig('IP',newW)}
  if (newX){window.elec.setConfig('Port',newX)}
  if (!newZ){
    window.elec.showUninstall().then((response)=>{
      if (response === 0) {installProxy()}
    })
  }
...
<template>
  <div id="frame" class="flex flex-col justify-end content-center">
    <div class="flex-none self-center w-2/3 h-14">
      <button @click="selectGamePath" id="selectbtn" class="flex content-center justify-center gap-x-2
                                    w-full h-full bg-amber-400 rounded-lg shadow-sm
                                    hover:bg-amber-300 active:bg-yellow-500">
        <svg id="selectSVG" class="self-center" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
             width="28" height="28">
          <path
        ...
        <p class="self-center font-sans text-lg font-semibold tracking-wide text-yellow-900 text-center align-middle">
          选择游戏可执行文件
        </p>
      </button>
    ...
    <div class="flex-grow bg-gray-600 bg-opacity-80">
      <div class="flex flex-nowrap flex-col justify-around content-center w-full h-full">
        <p class="font-sans text-base font-normal tracking-wide text-gray-100 text-center align-middle">
          <span v-if="!execpath">请选择游戏可执行文件</span>{{execpath}}
        </p>
        <div class="flex justify-around">
          ...
          <div class="w-1/4">
            <button @click="ipVerify" id="selectbtn" class="flex content-center justify-center gap-x-2
                                    w-full h-full bg-amber-400 rounded-lg shadow-sm
                                    hover:bg-amber-300 active:bg-yellow-500">
              <p class="self-center font-sans text-lg font-semibold tracking-wide text-yellow-900 text-center align-middle">
                查询
        ...
        <div class="flex flex-col border border-white rounded-xl">
          <p class="font-sans text-2xl font-normal tracking-wide text-gray-100 align-middle">
    ...
    <div class="flex-none w-full h-24">
      <button id="startbtn" @click="startGame" class="w-full h-full bg-amber-400 rounded-lg shadow-sm
                                  hover:bg-amber-300 active:bg-yellow-500 disabled:bg-gray-400" :disabled="!isActive">
        <p class="font-sans text-3xl font-semibold tracking-wide text-yellow-900 text-center align-middle">
          启动私人服务器
        </p>
      </button>
    </div>
